...
const [file,expectFile,limit]=process.argv.slice(1),expected=JSON.parse(await fs.readFile(expectFile));
...
console.log(JSON.stringify({triangles,meshes,maxPositionError,bounds:{min:bounds.min.toArray(),max:bounds.max.toArray()},bytes:original.length}));
'''
...
def export(args):
 ...
 with tempfile.TemporaryDirectory(prefix='.bike-export-',dir=generated.parent) as temp:
  ...
  try:
   pb=C.bake_atlas(body,size,str(textures),stem+'_body',jpeg_quality=86,normal_size=size//2,orm_size=size//2,variants=[(cw,lambda cw=cw:C.apply_colourway(colours[cw])) for cw in ('rookie','pro')])
   pm=C.bake_atlas(mech,size,str(textures),stem+'_mech',jpeg_quality=86,normal_size=size//2,orm_size=size//2)
   ...
   verified=subprocess.run([args.node,'--input-type=module','-e',VERIFY,str(stage/glb.name),str(stage/'expected.json'),str(args.lod_tris if args.lod else 0)],cwd=C.ROOT,check=True,capture_output=True,text=True)
   report=dict(source=str(source),sourceSha256=source_hash,exportSha256=digest(stage/glb.name),lod=args.lod,lodBudget=args.lod_tris if args.lod else None,verified=json.loads(verified.stdout))
