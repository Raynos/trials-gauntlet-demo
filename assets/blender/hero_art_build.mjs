...
export async function build(name, { lod, stage = 0, scratch, dry = false }) {
  ...
  const audit = {
    ...
    reduced: {
      ...
      raceCollar: reduce.raceCollar ?? null,
      atlas: reduce.atlas ? { size: reduce.atlas.size, parts: reduce.atlas.parts ?? null, hiresSources: reduce.atlas.hiresSources ?? null, hiresNormal: reduce.atlas.paths?.hiresNormal ?? null } : null,
    },
