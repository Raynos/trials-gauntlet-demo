...
The attached image is the CURRENT track-select screen of "Trials Gauntlet", a Trials-style 2.5D motorbike game, captured headless on a landscape iPhone (932x430 CSS px). It is a REFERENCE for the art style, the colours, the font, and the data on the cards. Do not copy its layout — the whole point is a NEW information architecture described below.

## Round 2 (what changed since the first five studies)
The user has chosen the BIOME MAP direction: the player browses the world by BIOME (Industrial, Canyon, Snow, Night City, Foundry) and the tiers (Beginner … Extreme) are a property of each track — its code letter and its lock — not the thing you browse. Every mockup in this round is a biome map; the direction brief at the end says which architecture. Three things the first round got wrong and this round must get right:
1. FRAME: the phone screen is EXACTLY 1536 x 708 image px (19.5:9), centred, with 158 px of plain black letterbox above and 158 px below. The first round drew squarer 2.0:1 screens — do not. The UI must fill 1536 x 708 edge to edge.
2. IN FRAME: the LAB and the PLAYGROUNDS must be visible inside the frame (as the different kind of thing the direction describes), AND at least one LOCKED track with its requirement text must be visible inside the frame (H1 Rooftop Wire with "NEXT UNLOCK · MEDAL EVERY MEDIUM TRACK"). Compose the visible span so that B1 (focused), M1 (UP NEXT) and H1 (locked) are all on screen at once.
