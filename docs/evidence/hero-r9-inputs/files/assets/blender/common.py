...
def local_ao_settings(scene):
    """Read a protected source's bake policy; old sources explicitly disable AO."""
    raw = scene.get('heroLocalAO')
    settings = json.loads(raw) if raw is not None else dict(distance=0.0, samples=32, strength=0.8)
    if not isinstance(settings, dict) or set(settings) != {'distance', 'samples', 'strength'}:
        raise RuntimeError('heroLocalAO requires distance, samples and strength.')
    distance, samples, strength = (settings[key] for key in ('distance', 'samples', 'strength'))
    if (type(distance) not in (int, float) or not math.isfinite(distance) or not 0 <= distance <= .10
            or type(samples) is not int or not 1 <= samples <= 256
            or type(strength) not in (int, float) or not math.isfinite(strength) or not 0 <= strength <= 1):
        raise RuntimeError('Invalid saved local AO: distance 0..0.10 metres, samples 1..256, strength 0..1.')
    return settings
