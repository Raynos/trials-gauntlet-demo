...
for(const name of ['bike','bike-lod']) {
 const actual=await load(`harness/out/blender/mega-bike/final-candidate/models/${name}.glb`),base=await load(`harness/out/blender/mega-bike/${name==='bike'?'ao-models':'protected-models'}/${name}.glb`);
...
await fs.writeFile('harness/out/blender/mega-bike/final-candidate/validation.json',JSON.stringify(reports,null,2));console.log(JSON.stringify(reports,null,2));
