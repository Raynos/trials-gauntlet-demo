...
OUT=ROOT/'race-lower-v2.blend';assert not OUT.exists()
...
b=C.MeshBuilder('rider:articulated race boots');SEG=26
for side,sg in (('L',-1),('R',1)):
 ...
 for row in range(14):
  ...
  for i in range(SEG):
   ...
   if row<4:
    ...
    pos=ankle+Vector(((.165 if c>=0 else .088)*c,.062*s,.020-.022*max(0,c)+.018*max(0,-c)));w={'foot.'+side:1}
   else:
    idx=row-5;u=(.040,.075,.110,.145,.190,.238,.280,.318,.328)[idx]
 ...
 for row in range(13):
  for i in range(SEG):
   ...
   elif row==2:mat=M['rubber'] if frontness<-.55 else M['shell']
   elif row in (3,4,5,6):mat=M['flex'] if row>4 or frontness<-.3 else M['shell']
   elif row>=7:mat=M['plate'] if frontness>.35 else M['shell']
   else:mat=M['shell']
   if row in (10,11) and -.35<frontness<.38:mat=M['color']
...
pants.data.materials[0]=R.material('race technical pant body',(.035,.075,.12),.81,750,'PA')
