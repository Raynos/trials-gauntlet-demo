...
OUT=ROOT/'street-anatomy-v3d.blend'
...
bm=bmesh.new();arm_layer=bm.verts.layers.float.new('anatomical_shoulder_weight');lower_layer=bm.verts.layers.float.new('anatomical_elbow_weight')
vs=[bm.verts.new(Vector(p)) for p in j['vertices']]
regions=[[] for _ in vs]
for face,region in zip(j['faces'],j['face_sets']):
 if region in (1,11,12,17,18,19,20,21):
  for index in face:regions[index].append(region)
for v,rr in zip(vs,regions):
 if rr:
  v[arm_layer]=sum(r in (11,12,20,21) for r in rr)/len(rr)
  v[lower_layer]=sum(r in (11,12) for r in rr)/len(rr)
for face,region in zip(j['faces'],j['face_sets']):
 if region in (1,11,12,17,18,19,20,21):bm.faces.new([vs[i] for i in face])
bmesh.ops.delete(bm,geom=[v for v in bm.verts if not v.link_faces],context='VERTS')
for z,no in ((.900,Vector((0,0,1))),(1.387,Vector((0,0,-1)))):
 geom=list(bm.verts)+list(bm.edges)+list(bm.faces)
 if z<1:
  geom=[v for v in bm.verts if abs(v.co.x)<.23]+[e for e in bm.edges if all(abs(v.co.x)<.23 for v in e.verts)]+[f for f in bm.faces if all(abs(v.co.x)<.23 for v in f.verts)]
 bmesh.ops.bisect_plane(bm,geom=geom,dist=.000001,plane_co=Vector((0,0,z)),plane_no=no,clear_inner=True,clear_outer=False)
# Diffuse only across authored anatomical seams, retaining a local shoulder transition.
for _ in range(3):
 for layer in (arm_layer,lower_layer):
  values={v:.55*v[layer]+.45*sum(e.other_vert(v)[layer] for e in v.link_edges)/max(1,len(v.link_edges)) for v in bm.verts}
  for v,value in values.items():v[layer]=value
...
for v in bm.verts:
 ...
 armblend=max(0,min(1,v[arm_layer]))
 lower=max(0,min(1,v[lower_layer]/max(.001,armblend)))
