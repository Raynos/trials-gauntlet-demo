...
const rows=[];
for(const kpsi of [1500,2500,4000])for(const cpsi of [180,260,360,500])for(const tauMax of [500,800,1200]){
 const over={rider:{kpsi,cpsi,tauMax}};
 const rookie8=labRun('rookie',8,over),rookie9=labRun('rookie',9,over);
 const pro8=labRun('pro',8,over),pro9=labRun('pro',9,over);
 rows.push({over,rookie8,rookie9,pro8,pro9,clears:[rookie8,rookie9,pro8,pro9].filter(r=>r.cleared&&!r.fault).length});
}
rows.sort((a,b)=>b.clears-a.clears);
await writeFile(new URL('./lab-tune.json',import.meta.url),JSON.stringify(rows,null,2)+'\n');
console.info(JSON.stringify(rows.slice(0,8),null,2));
