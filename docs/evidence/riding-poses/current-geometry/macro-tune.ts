...
import { stepN, runController, lipHopper } from '../../../../src/physics/controllers';
...
function labRun(cls: BikeClassV2, speed: number, over?: PartialTuningV2): { cleared: boolean; margin: number; fault: string | null; landPitch: number; apex: number; cause:string|null; faultX:number; reach:{arm:number;leg:number};pose:unknown } {
  ...
  const r = runController(w, lipHopper(lipX, speed, .3, .22, .1, true, {snapLead,preLean}), {
    ticks: HZ * 16,
    decisionHz: 60,
    latencyMs: 50,
    onTick: (s, input) => {
  ...
  if (process.env.POSE_RECORD) recordings.push({cls,speed,snapLead,preLean,inputs});
...
let snapLead=1,preLean=-.8;
const rows=[];
for(const lead of (process.env.POSE_RECORD ? [1,.8] : [.2,.4,.6,.8,1,1.2,1.4,1.6,1.8,2]))for(const preload of (process.env.POSE_RECORD ? [-.8] : [-.5,-.8,-1])){
 snapLead=lead;preLean=preload;
 const rookie8=labRun('rookie',8),rookie9=labRun('rookie',9),pro8=labRun('pro',8),pro9=labRun('pro',9);
 rows.push({lead,preload,rookie8,rookie9,pro8,pro9,clears:[rookie8,rookie9,pro8,pro9].filter(r=>r.cleared&&!r.fault).length});
}
rows.sort((a,b)=>b.clears-a.clears);
await writeFile(new URL('./macro-tune.json',import.meta.url),JSON.stringify(rows,null,2)+'\n');
console.info(JSON.stringify(rows.slice(0,3),null,2));

if(process.env.POSE_RECORD) await writeFile(new URL(`./lab-technique-recordings-${process.env.POSE_RECORD}.json`,import.meta.url),JSON.stringify({note:'Old and candidate controller inputs on the shared-geometry candidate physics; margin retains historical 1.6m reference, actual obstacle1.5m. Both techniques use identical entry, obstacle, survival condition.',rows,recordings},null,2)+'\n');
