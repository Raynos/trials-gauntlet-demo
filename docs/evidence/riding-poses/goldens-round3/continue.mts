...
const recorder=new InputRecorder(recordingHeader(sim,`bot skill=3 continued-from=${path.basename(prefixFile)} planner-history-reset=true outcome=${play.outcome} prefix-method=${JSON.stringify(prefix.header.note ?? "unspecified")}`));
