...
**Request to the parent (for #3):** the renderer only sees a DPR, and DPR 2 is also a retina Mac. `GameRenderer.setDeviceClass?(c: 'phone' | 'desktop')` called from the app with `isPhone()` (default `desktop`; the harness bench sets it on `window.__render` directly) lets `high` mean the phone-high definition above on a phone and stay the desktop tier on a desktop — one tier name, two budgets, no `src/game` logic beyond the one call.
