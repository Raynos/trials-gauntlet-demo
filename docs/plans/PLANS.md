...
| **Blender hero (branch)** | `docs/plans/BLENDER_HERO.md` | rider/bike models, animation and Three.js integration to the Trials reference; Codex owns Blender on `blender-work` | R10 integrated baseline; R11 four independent presets built, production boot bug fixed, all 16 preset/bike/LOD runtime combinations pass; first procedural blockout generated; initial head/hair sampling defects corrected, second geometry gate pending; parent accepted corrected charcoal palette only; first visible procedural blockout needs coherent silhouette/attachment correction | — | implement all five approved independent outfits through img2threejs; surface/likeness below targets; actual iOS and known R7 physics regressions open; no merge |
