...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Shared TypeScript / Three.js Capacitor apps; offline installation, store releases and controlled OTA required for first release | **in build** — personal publisher, free/no ads. Simulator matrix is required for goal completion. Both shells compile; installed gameplay, lifecycle, durable saves, signed OTA and local upgrades have scoped evidence. Round9 adds dark Android system bars and a durable failed-activation ledger; types/lint, 1,169 tests (two existing skips) and builds pass. iOS terminal ship/native-touch checks pass; Android draw latency remains a separate measured concern. API24 stock WebView53 shows the failure page but cannot play. [Latest evidence](../evidence/native-mobile/round9.json) | — | Remaining simulator/device matrix and Android draw timing, older OS/provider qualification, changed-controller OTA/low-space recovery, whole-device pressure, signed upgrades/production channels and HR-14 store prerequisites |
