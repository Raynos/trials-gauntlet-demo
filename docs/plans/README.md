...
| **World map level select** | [WORLD_MAP.md](WORLD_MAP.md) | the A "Rebirth" painted continent as the level select — pan / zoom over one continuous terrain, markers with leader-line plates, the route as a lit road, fog over locked regions, drifting cloud shadows; the diorama retired | **live** — R1+R2 shipped `a32d7e0` (plates, camera, markers, roads, gate, fog, card; diorama deleted); R3 polish vs the mockup in flight | 70 | plan doc + terrain plates first; played evidence at every round |
