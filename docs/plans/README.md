...
| **World map level select** | [WORLD_MAP.md](WORLD_MAP.md) | the A "Rebirth" painted continent as the level select — pan / zoom over one continuous terrain, markers with leader-line plates, the route as a lit road, fog over locked regions, drifting cloud shadows; the diorama retired | **in build** (ask 50, started 2026-09-17) | 0 | plan doc + terrain plates first; played evidence at every round |
...
| 09-17 | **The round-4/5 world map is rejected** ("dioramas linked on one page") — a **from-scratch world map** in the language of FF VII Rebirth / FF XV / FF XVI / Black Desert Online; three codex mockups first (ask 47), the user picks, then it is built new, not refactored from the tiles |
| 09-17 | **Level select → A "Rebirth"** (`assets/design/worldmap/A-painted-world.png`): "literally that" — a new `WorldMapScreen` built from scratch, the diorama retired — ask 50, plan `docs/plans/WORLD_MAP.md` |
