...
| **Store release** | [STORE_RELEASE.md](STORE_RELEASE.md) | Ship to Google Play + the App Store (iPhone) as an original product: new name, zones, HUD, 12 new tracks, locally generated music; same gameplay | **in build — live on the web as ROCKHOP (asks 86/95/98/104)**: 12 decisions taken 2026-09-22; Play closed test (12 testers × 14 days) is the critical path | 0 | P0 clean room + HR-16 accounts |
...
| **Riding poses** | [RIDING_POSES.md](RIDING_POSES.md) | seated neutral, forward, back, natural elbows, explicit contact release, runtime cloth — in played clips | **in final integration (ask 82)**: 50/50 exact finishing Node/browser pose goldens on final simulation `2249b7bb` across 26 pre-Rockhop courses; all 12 original blind B1–E3 recordings re-proved on the final source, with only Rockhop course files changed since the sessions. Silent Rookie/Pro motion video, 20-subject transition/thrown actual-skin audits and low/high headless WebKit phone-geometry pass. Rockhop's `a7ed0716` passed CI and production serves that SHA. Shared in-progress UI worktree's silent Metal gate reached 29/31: pose gameplay/replay/camera/render rows pass; bundle and retained heap miss, and offline map needs its fifth region plate | ~90% of pose release readiness | fix integrated UI/gate misses, accept all six behaviors on played evidence, push the pose evidence/recordings and verify the resulting production SHA |
