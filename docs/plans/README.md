...
"done" lines, except closures explicitly accepted by the user with recorded exceptions. Live build: https://trials-gauntlet-demo.vercel.app · pins v0.1.0 / v0.2.0 / v0.2.1 / **v0.2.2** (the garage build):
...
The post-merge handoff `HERO_OPEN_WORK.md` was split on 2026-09-16 (archived): [RIDING_POSES.md](RIDING_POSES.md) (seated / forward / back / elbows / contacts — gameplay), `project/archive/CHROMIUM_METAL_SHADER_INIT.md` (the GL 1281 startup bug — closed as non-repro the same day) and [HERO_ART_INTEGRATION.md](../../project/archive/HERO_ART_INTEGRATION.md) (Astra's R33/R34 delivery into the game, closed on the user's acceptance in ask 71). Riding poses remain open.

## Garage production — historical September 17 round log
...
**September 15 status check (historical):** HEAD `5b56431` plus in-flight R8 changes, fingerprint `089e0885`. The matching on-disk quick gate is 27/30 (three SwiftShader timing failures; determinism passes, logical restart one tick). Fresh targeted tests give 15/16: the R6 hop-force assertion fails at 1398.931 > 1280; no fresh full-suite pass is claimed. The game still uses custom v2; no physics library is installed. The user-requested migration plan lives in [USE_A_REAL_PHYSICS_LIBRARY.md](USE_A_REAL_PHYSICS_LIBRARY.md): Rapier 2D first, Planck comparison/fallback, implementation not started. AI strangers remain solvability evidence distinct from human learnability; archived plan percentages do not close the mission bars.

**September 21 evidence snapshot:** the locally generated reflex sweep now tracked in `harness/out/metrics/` has two seeds per track, source fingerprint `4b9bc981`; it is narrower than the earlier nine-seed sweep and does not replace a human learnability check. The nine generated C Ascent seam/source PNGs are tracked under `assets/design/tracks/round4/build/out/` (ask 69).
...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Package the shared TypeScript / Three.js game with Capacitor; offline installation, store releases and a controlled web-update channel required for first release | **in build** — ask 73 autonomous implementation; Capacitor shells compile, native boot/lifecycle/saves and signed OTA implemented; local installed-app staging, activation, watchdog rollback and save recovery proved; five iOS profiles pass gameplay smoke and primary iOS/Android pass all-track/garage asset probes; five iOS profiles pass control geometry, orphan cleanup passes on iOS, Android native-touch/lifecycle/save checks pass after fixing missed quick restart taps, and iPhone17e XCTest touch/lifecycle smoke passes; repeatable installed OTA suites pass 17 iOS stages and 18 Android checks including interrupted retry, negative releases, orphan cleanup and watchdog fallback with preserved preferences; local binary upgrades preserve PB/ghosts and preferences on both platforms; runtime save failures now warn and retry with installed filesystem-failure proof. Graphics loss now pauses the game until renderer recovery and explicit Resume, proved on both final artifacts with sixth-round ship gates; native Release builds and branded packaging audit pass within development scope. Round7 actual permission-denied writes and reads preserve committed saves and recover after access is restored on both platforms. Bounded iOS ENOSPC also preserves saves and recovers through Retry. API24 stock WebView53 fails game boot; the shell now shows a verified readable error page, with healthy iOS26.5/API36 regression smokes passing. Physical-device/store qualification remains open. Personal publisher, free/no ads, OTA in first release | — | Required iOS26.5 phone/tablet + Pixel7/API36 E2E matrix (ask76); OS low-storage, older WebView qualification, final signed upgrades and remaining device gates; account/production-channel/store prerequisites HR-14 |
...
| **Offline PWA** | [PWA_OFFLINE.md](../../project/archive/PWA_OFFLINE.md) | one online load, then an offline cold start plays the game from the iPhone home screen; a warm start never waits on the wire | **done — archived**: headless origin-down gate reaches 100/100, finishes b1 bit-identically, swaps 10/10 garage combinations with 0 network requests, and passes the slow-link warm start; user confirmed the PWA works offline on the actual phone on 2026-09-21; tag `pwa-offline-complete` | 100 | — |
| **World map level select** | [WORLD_MAP.md](../../project/archive/WORLD_MAP.md) | the A "Rebirth" painted continent as the level select — pan / zoom over one continuous terrain, markers with leader-line plates, the route as a lit road, fog over locked regions, drifting cloud shadows; the diorama retired | **done — archived on user acceptance (ask 71)**: seven machine-evidenced lines pass; Canyon framing fixed at `411697e`; actual iPhone gestures were waived for closure and remain unmeasured | 100 (accepted scope) | — |
...
| **Hero art integration** | [HERO_ART_INTEGRATION.md](../../project/archive/HERO_ART_INTEGRATION.md) | the R33 catalog rider + bike, five outfits, two liveries in the garage and in-level within the phone-high rows; garage 30 fps on the user's iPhone | **done — archived on user acceptance (ask 71)**: Astra is the only hero family, the prototype is retired, loader and ship gate passed, and `v0.3.0` is pinned; phone-high 139 calls / 9.7 ms still misses the old ≤ 123 / ≤ 8.8 bar and is carried in the perf backlog; actual iPhone garage fps remains unmeasured | 100 (accepted scope) | performance gap: [PERF-BACKLOG.md](PERF-BACKLOG.md) |
...
| 09-17 | **Level select → A "Rebirth"** (`assets/design/worldmap/A-painted-world.png`): "literally that" — a new `WorldMapScreen` built from scratch, the diorama retired — ask 54, plan `project/archive/WORLD_MAP.md` |
...
| 09-17 | (live `0788a68`) **the offline PWA** (ask 58, `bff8ef1`): one online load caches the whole game (180 entries / 41.96 MB incl. 14 models), a cold start with no network plays (0 requests, b1 golden bit-identical, garage 10/10 with no network), the update toast is gone (update boot 39.8 KB), everything up front (DOWNLOAD 38.44 MB), versioned art + 1 y / 1 month headers, iOS icons and splashes; gate 27/31 with `offline.coldStartPlayable` PASS | `--archive=tgz`; the deploy caught `$comment` keys `vercel.json` rejects (`0788a68`) | `bff8ef1` `0788a68` |
| 09-18 | (live `bde6ff7`-era HEAD) **ask 59's five quick wins**: deploy 73.23 → 46.34 MB (26.89 MB of flat model copies pruned), first boot 39.01 → 37.08 MB on a DPR-2 phone and → 35.73 MB on a 1x desktop (one tier per device, no 2x art on 1x, `og.jpg` out of the pack), the Canyon back in the phone's opening frame; gate 27/31 with `offline.coldStartPlayable` PASS | `--archive=tgz` | `fbbcf5f` `411697e` `da8b8f4` |
...
| 09-17 | **Offline PWA, the parent's two defaults overruled by the user:** (a) **everything loads up front** — no background or just-in-time fill; the first boot pulls the whole ≈ 34 MB while the loader is up ("I want this to behave like a game. Load everything up front, but aggressively cache it … second boot, all cache, all cache, all cache"); if that is slow, **the fix is the loading, not the strategy** (a separate benchmarking task). (b) **The update toast is deleted.** A build that needs updating activates and reloads **at the very start of the loading screen**, before the bars move, so the player sees one loading screen and is then on the new build — never "downloaded everything, now press refresh to download it again" |
