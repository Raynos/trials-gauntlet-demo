...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Shared TypeScript / Three.js Capacitor apps; offline installation, store releases and controlled OTA required for first release | **in build** — personal publisher, free/no ads. Both shells compile; installed gameplay/assets, touch/lifecycle, durable saves, signed OTA activation/rollback and local binary upgrades have scoped evidence. Graphics loss pauses safely. Permission failures and bounded save-path ENOSPC recover on both platforms; iOS corruption/future-version/pending-file recovery passes. API24 stock WebView53 receives a readable failure page; gameplay remains unqualified there. Round8: full repository check passes (1,156 tests, two existing skips) and offline PWA suite passes10/10. [Latest evidence](../evidence/native-mobile/round8.json) | — | Remaining simulator/device matrix, older OS/provider qualification, low-space OTA and whole-device pressure, final signed upgrades/production channels and HR-14 store prerequisites |
