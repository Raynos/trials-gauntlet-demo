...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Package the shared TypeScript / Three.js game with Capacitor; offline installation, store releases and a controlled web-update channel required for first release | **in build** — ask 73 autonomous implementation; Capacitor shells compile, native boot/lifecycle/saves and signed OTA implemented; local installed-app staging, activation, watchdog rollback and save recovery proved; physical-device/store qualification remains open. Personal publisher, free/no ads, OTA in first release | — | Integrated offline/save/OTA recovery gates; account/device/store prerequisites HR-14 |
