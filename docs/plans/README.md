...
| **Chromium/Metal shader init** | `project/archive/CHROMIUM_METAL_SHADER_INIT.md` (closed 2026-09-16) | zero GL errors on a Chromium/Metal cold boot → clear → crash → restart | **done — non-repro, archived**: 18/18 unmodified Metal gate runs clean, including the exact 09-15 failing build and the commit before r15; not a code defect — machine state on the 09-15 evening. Residual: fully cold `MTLCompilerService` cache = HR-08 | 100 | [evidence](../evidence/chromium-metal/README.md) |
