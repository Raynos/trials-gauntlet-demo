...
| **Store release** | [STORE_RELEASE.md](STORE_RELEASE.md) | Ship to Google Play + the App Store (iPhone) as an original product: new name, zones, HUD, 12 new tracks, locally generated music; same gameplay | **planned — top priority (ask 86)**: 12 decisions taken 2026-09-22; Play closed test (12 testers × 14 days) is the critical path | 0 | P0 clean room + HR-16 accounts |
| **Use a real physics library** | [USE_A_REAL_PHYSICS_LIBRARY.md](USE_A_REAL_PHYSICS_LIBRARY.md) | Ship **Custom v2 + a library engine** in the same game, toggled on the main menu so the player can exit, switch and compare the same level/bike | **parked until the store release ships (ask 86)**; planned, not started — Rapier 2D first candidate, Planck comparison/fallback; retain both engines after release, persist selection and separate engine records; no package/adapter landed | 0 | P0: coordinate in-flight R8 work and freeze a reproducible control; P1: 3-5 day qualification; P2: main-menu selector and both playable engines |
...
| **Riding poses** | [RIDING_POSES.md](RIDING_POSES.md) | seated neutral, forward, back, natural elbows, explicit contact release, runtime cloth — in played clips | **in qualification (ask 82)**: 50/50 exact Node/silent-browser finishing replays on 26 pre-Rockhop courses at source `207404d3` (47 bot controls, three stranger-derived); fresh M2 Pro bot and 12/12 new blind B1–E3 clears on this exact source. Silent Rookie/Pro transition video recorded. Current-code cloth playback: 20 outfit/detail/class combinations, 1,700 transition and 2,490 thrown pre-release sampled frames, no overlap beyond 5 mm; WebKit phone-geometry low/high hero pass. Latest clean-main CI: 1,344 tests pass, three fail on stale Rockhop golden finish and changed offline-pack byte count; deploy has never reached production. Earlier Metal cold boot/clear/crash/restart/offline pass, old physics pins 6/6, synced GPU readback timing variable | ~85% of pose release readiness | qualify final Rockhop recordings, clear integrated tests and full silent Metal gate, then deploy and verify production SHA |
