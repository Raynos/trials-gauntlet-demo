...
`prototypes/hero-garage/` contains the independent Three.js viewer, provisional identity board, source/license ledgers, editable Blender sources and recorded head trials. Build and frozen WebKit loading/orbit, byte-identical canvas repeat, missing-file and touch checks pass. **Milestone A remains blocked:** the generic anatomy route missed identity/hair; the changed textured-scan route improved skin but still fails eye reconstruction and curly-hair quality after correction by a fresh owner. The parent rejected the final [comparison clip](../../prototypes/hero-garage/captures/head-scan-a2-review/webkit-orbit-relight.webm); [art handoff](../../prototypes/hero-garage/reports/ART-HANDOFF.md) is ready. No body/bike/rig/variants/game integration or actual iPhone pass is claimed.
