...
| **Hero art integration** | [HERO_ART_INTEGRATION.md](HERO_ART_INTEGRATION.md) | the R33 catalog rider + bike, five outfits, two liveries in the garage and in-level within the phone-high rows; garage 30 fps on the user's iPhone | **in flight (ask 43, opened 2026-09-16)** — art-pipeline owner: game-budget exports (groom → cards, ≤ 60 k tris, LODs) into `public/models/`; render owner: per-outfit / per-livery loader, rig binding, clip aliasing; HR-07 stays the visual bar, not a gate on the engineering | 40 | round 1 landed: 14 game-budget GLBs, Astra family live (`LEGACY_HERO` one line away), rider LOD on phone tiers in-level, played garage/b1 evidence in `docs/evidence/hero-art/round1/`; next: hands-on-grips residual vs the legacy rider, the 3G boot stall, the user's hair verdict, then legacy files + prototype retired |
