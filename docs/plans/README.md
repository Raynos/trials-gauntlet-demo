...
| **Offline PWA** | [PWA_OFFLINE.md](PWA_OFFLINE.md) | installed to the iPhone home screen, aeroplane mode, cold start → the game loads and a track is playable; a warm start never waits on the wire | **planned, not started** (ask 58) — a SW and manifest already exist; the first load throws away the 27 MB it just fetched (registration lands after the boot has requested the 14 hero GLBs, `PRECACHE_PHASES` is core+title only) and every deploy wipes the cache (`Date.now()` in the cache key) | 0 | R1: register before the boot fetches + split shell / immutable caches + `harness/e2e/offline.mts`; the fix costs zero extra bytes |
