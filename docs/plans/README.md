...
`prototypes/hero-garage/` contains the independent Three.js viewer, provisional identity board, source/license ledgers, editable Blender sources and recorded head trials. Build and frozen WebKit loading/orbit, byte-identical canvas repeat, missing-file and touch checks pass. **Milestone A remains unaccepted:** the generic anatomy route missed identity/hair; the changed textured-scan route improved skin but still fails eye reconstruction and curly-hair quality after correction by a fresh owner. The parent rejected the final [comparison clip](../../prototypes/hero-garage/captures/head-scan-a2-review/webkit-orbit-relight.webm); [art handoff](../../prototypes/hero-garage/reports/ART-HANDOFF.md) is ready. The MPFB trial improves open eyes/topology but still fails identity, hair and skin/beard in recorded review. The main Blender/MPFB plan now proceeds independently: correction2 improves beard fit and eyes but still fails; the exhausted Cortu hair source is being replaced with a licensed Bystedt curl groom; alignment is fixed but the first orbit rejects sparse coverage and wiry curls; correction2 fixes penetration and strand conversion but still fails crown coverage and silhouette. Conversion-only tuning has stopped; authored-guide redistribution and combing are underway. Hunyuan is a separate parallel experiment; TRELLIS is paused. Round3, round6 and round9 existing-game coldboot/byte-identical clear/crash/restart pass. No body/bike/rig/variants/game integration or actual iPhone pass is claimed.
...
| **Hero merge and garage prototype** | `docs/plans/HERO_GARAGE_PRODUCTION.md` | Merge playable rider improvements; independently produce one high-fidelity Blender character/bike | Merged into local main at `62c003d`: five outfits, 814 tests and WebKit flat/B1 byte-identical clear/crash/restart pass. R7 physics retained. Seated/lean/elbow candidate isolated after 15 handling failures; standalone viewer built; two head routes rejected; MPFB trial also unaccepted; MPFB correction2 rejected; Bystedt groom orbit rejected; groom correction2 still rejected; authored-guide regroom active; independent Hunyuan experiment | — | Separate physical retuning of seated candidate and standalone head/curly-hair milestone; Metal shader-query issue remains documented |
