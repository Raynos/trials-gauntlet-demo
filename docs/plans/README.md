...
The post-merge handoff `HERO_OPEN_WORK.md` was split on 2026-09-16 (archived): [RIDING_POSES.md](RIDING_POSES.md) (seated / forward / back / elbows / contacts — gameplay), [CHROMIUM_METAL_SHADER_INIT.md](CHROMIUM_METAL_SHADER_INIT.md) (the GL 1281 startup bug) and [HERO_ART_INTEGRATION.md](HERO_ART_INTEGRATION.md) (Astra's R33/R34 delivery into the game — ours). The five-outfit merge does not close any of them.
...
| **Chromium/Metal shader init** | [CHROMIUM_METAL_SHADER_INIT.md](CHROMIUM_METAL_SHADER_INIT.md) | zero GL errors on a Chromium/Metal cold boot → clear → crash → restart | **open, possibly moot** — last seen on the R15 frozen build; r15's `ResourceRetirement` (`5b56431`) has not been re-checked against it | 0 | repro on the current build, no wrappers |
| **Hero art integration** | [HERO_ART_INTEGRATION.md](HERO_ART_INTEGRATION.md) | the R33 catalog rider + bike, five outfits, two liveries in the garage and in-level within the phone-high rows; garage 30 fps on the user's iPhone | **not started** — waits on HR-07 (the user's art review) and asks #30–31 (classic / img2 removal) | 0 | loader path for the packed rider as the replacement family |
