...
`prototypes/hero-garage/` contains the independent Three.js viewer, provisional identity board, source/license ledgers, editable Blender sources and recorded head trials. Build and frozen WebKit loading/orbit, byte-identical canvas repeat, missing-file and touch checks pass. **Milestone A remains unaccepted:** the generic anatomy route missed identity/hair; the changed textured-scan route improved skin but still fails eye reconstruction and curly-hair quality after correction by a fresh owner. The parent rejected the final [comparison clip](../../prototypes/hero-garage/captures/head-scan-a2-review/webkit-orbit-relight.webm); [art handoff](../../prototypes/hero-garage/reports/ART-HANDOFF.md) is ready. The MPFB trial improves open eyes/topology but still fails identity, hair and skin/beard in recorded review. User-directed TRELLIS.2 local Metal setup passes; weights are downloading and DINOv3 access awaits user login; Hunyuan3D2.1 license excludes the user’s declared territory. Round3 existing-game coldboot/byte-identical clear/crash/restart passes. No body/bike/rig/variants/game integration or actual iPhone pass is claimed.
...
| **Rider on Glass** | `docs/plans/RIDER_ON_GLASS.md` | the rider and bike are the hero; the game is proven on a phone | G closed; H: hero merged + `riderBody` live (`fe50df5`); **critic r4 = 1/6 + 1/6 (2/12, both crash cells)** — the statue tells are gone; what remains is the R8 landing punch (rider through the tank, 4/12) and the camera (9/12) | **G 100 · H open, bar 2/6 missed by one** | physics R8 (landing constraint + thrown-rider fault, brake endo, strangers n ≥ 2) ∥ render r15 (lead / landing-tighten / occluder / near-plane pop, free camera, playground set pieces) → critic r5 |
