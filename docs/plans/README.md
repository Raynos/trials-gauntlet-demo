...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Shared TypeScript / Three.js Capacitor apps; offline installation, store releases and controlled OTA required for first release | **implementation built; private store testing prioritized; paused after round10 commit; collaboration branch `docs/native-mobile-publishing`** — personal publisher, free/no ads. Round10 passes9 iOS and12 Android updater checks: identical-publication retry, healthy journal clearing, foreground watchdog recovery and failed-version quarantine. Android proves actual bounded ZIP-write ENOSPC with saves preserved. Normal apps restored; test devices/server stopped. Fresh [gameplay clips](../evidence/native-mobile/mini-trailers.json) captured on both platforms; the iOS viewing crop removes the simulator Dynamic Island while the original device recording remains. Private upload waits on signing/account setup and permanent IDs; latest check still shows zero Apple signing identities and Android AAB unsigned. Latest product-source checks:1,169 tests/two existing skips; changed fixture tests and lint pass. [Evidence](../evidence/native-mobile/round10.json) | — | iOS OTA low-space/extraction and background-before-ready cases; remaining simulator/older-OS and physical-device gates; production channels, signed store builds and HR-14 publisher prerequisites |
