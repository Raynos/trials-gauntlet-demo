...
| **World map level select** | [WORLD_MAP.md](WORLD_MAP.md) | the A "Rebirth" painted continent as the level select — pan / zoom over one continuous terrain, markers with leader-line plates, the route as a lit road, fog over locked regions, drifting cloud shadows; the diorama retired | **live, device check open** — R1–R3 built the plates, camera, markers, roads, gate, fog and card; diorama deleted; ask 59.4 fixed the Canyon in the phone opening frame; all seven headless done lines pass, but actual iPhone pan/pinch has no report | 85 | HR-13: iPhone Safari pan, pinch, select and return; then judge archive readiness |
...
| **Hero art integration** | [HERO_ART_INTEGRATION.md](HERO_ART_INTEGRATION.md) | the R33 catalog rider + bike, five outfits, two liveries in the garage and in-level within the phone-high rows; garage 30 fps on the user's iPhone | **built, acceptance open** — Astra is the only hero family, legacy files and prototype retired, loader and ship gate passed, and v0.3.0 is pinned; measured phone-high rows miss the plan's ≤ 123 calls / ≤ 8.8 ms bar (integration 142 / 9.2; ask 60 139 / 9.7), and the new garage has no actual iPhone 30 fps reading (HR-10) | 60 | Re-measure phone-high against the plan's bar; HR-10 garage and hair reading; then judge archive readiness |
