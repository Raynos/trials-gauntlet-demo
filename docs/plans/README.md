...
| **Native iOS / Android publishing** | [NATIVE_MOBILE_PUBLISHING.md](NATIVE_MOBILE_PUBLISHING.md) | Package the shared TypeScript / Three.js game with Capacitor; offline installation, store releases and a controlled web-update channel required for first release | **in build** — ask 73 autonomous implementation; Capacitor shells compile, native boot/lifecycle/saves and signed OTA implemented; local installed-app staging, activation, watchdog rollback and save recovery proved; five iOS profiles pass gameplay smoke and primary iOS/Android pass all-track/garage asset probes; five iOS profiles pass control geometry, orphan cleanup passes on iOS, Android native-touch/lifecycle/save checks pass after fixing missed quick restart taps, and iPhone17e XCTest touch/lifecycle smoke passes; repeatable installed OTA suites pass 17 iOS stages and 18 Android checks including interrupted retry, negative releases, orphan cleanup and watchdog fallback with preserved preferences; physical-device/store qualification remains open. Personal publisher, free/no ads, OTA in first release | — | Required iOS26.5 phone/tablet + Pixel7/API36 E2E matrix (ask76); low-storage/save-upgrade and remaining device gates; account/production-channel/store prerequisites HR-14 |
