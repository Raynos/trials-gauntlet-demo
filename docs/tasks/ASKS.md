...
| 36 | "Main menu: more variants of B Lobby — larger GARAGE / PLAY / SETTINGS / REVIEW buttons; leak nothing about level, session, next up, cleared; no massive hero card — a cool midair jump in the not-yet-built Nalati grassland biome instead; 3 new variants" | **in flight** | design round `assets/design/menu/round3/`, B1–B3; the user picks |
| 38 | "It's definitely C Ascent for the levels / world map. The world map can be panned, dragged and zoomed — no need to cram all 15 levels into a static view: show a subset in the viewport and move the map to see the rest, or load the viewport centred on the current level wherever it is on the map" | **in flight** | build: the level select becomes the C Ascent continuous world (one mountain Lab → Foundry), drag to pan, pinch / wheel to zoom, opens centred on the current level; `src/ui/trackMap.ts` + `front.ts` |
