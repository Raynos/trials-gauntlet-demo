...
| `npx tsx harness/e2e/hero-art-clip.mts [--out DIR] [--engine webkit\|chromium] [--geom 874x330] [--dpr 3] [--ride-ticks N] [--quality high] [--fast]` | **played hero clip** (ask 43): one WebKit page at phone geometry recorded start to finish — boot → GARAGE tapped → every outfit tapped and PRO / ROOKIE on each (a per-rAF probe fails the run on any swap frame with the stage off, a procedural stand-in or no glTF rider = the ask 29/40 grey flash) → ‹ Menu → the same page in `?harness=1` → b1 from the Rookie golden as Street mustard, then from the Pro golden as Race blue/white, paced to real time, the finish hash checked against the node replay (and the Pro pin). Exit = failure / flash / page error / hash drift | `out/hero-art-clip/{clip.webm,clip.mp4,sheet.jpg,log.json}` |
