...
| `pnpm harness:audio [--seed N] [--coin balanced\|plain] [--bike pro] [--beats-only\|--pairs-only] [--out dir]` | **audio A/B stage (round 11, P6)**: renders `src/audio/tools/beats.ts`'s four beats (start-gate, wheelie, landing-2m, crash-respawn; byte-identical, the measured table printed against `reference/evolution-gameplay/audio/*.wav`) and builds six sealed AUDIO pairs `apair-<tag>-<stamp>-<hex>`: a black picture with only the A/B bar and clock, audio A / 1 s / B, `-A.wav` / `-B.wav` loudness-matched to −23 LUFS with one linear gain each, a two-spectrogram sheet, `.answer.json` chmod 000; `--coin balanced` (default) seats ours on A for 3 of 6. Then `harness:critic-prompt <apair-id>` (audio rubric) and `harness:log-verdict` as for clip pairs | `out/audio-beats/`, `out/compare/apair-<id>{.mp4,-A.wav,-B.wav,-sheet.jpg,.answer.json,.prompt.md}` |
...
| `npx tsx harness/e2e/offline.mts [--verbose=1]` · `pnpm harness:e2e --only=offline` | **the aeroplane-mode gate** (ask 58, `docs/plans/PWA_OFFLINE.md` §7): ONE online load on a `launchPersistentContext` profile, then the browser process AND the HTTP server are shut down and the game must cold-start and ride from Cache Storage. Nine checks: `cacheAfterFirstLoad` (≥ 14 `/models/`, ≥ 27 MB), `coldStartPlayable` (100/100, `transferSize=0`, `workerStart>0`, **zero** requests reached the origin), `firstFrame`, `rideFinishes` (the c1 golden's finish time and hash, offline == online: the shipped game's first course, not the retired set's `?`-URL dev chunk), `worldMapDraws` (the world plate and every drawn region plate, ≥ 4 zones), `inboxQueues`, `noPageErrors`, `slowStart` (50 kbps / 400 ms RTT within ×1.6 of the offline time) and `updateSurvives` (a new build's worker adopted at the start of the loading screen; 0 bytes of models re-fetched). Three rules the measurements taught: a persistent profile (an ephemeral context loses Cache Storage), the **server stopped** (Playwright's `offline` flag does not reach service-worker fetches, so the HTTP disk cache answers and the suite lies), and wire bytes counted **in the server** (the worker's requests never reach a page CDP session). The only suite that runs WITH the worker — everything else passes `?sw=0` — so `offline.coldStartPlayable` is also a ship-gate row | `out/offline/offline.json` |
| `npx tsx harness/e2e/hero-art-clip.mts [--out DIR] [--engine webkit\|chromium] [--geom 874x330] [--dpr 3] [--ride-ticks N] [--quality high] [--garage-quality high] [--fast]` · `pnpm harness:e2e --only=heroart` (the same as an opt-in e2e flow: it needs WebKit, its own recorder and ~2 min of real-time rides, so the default e2e set does not include it) | **played hero clip** (ask 43): one WebKit page at phone geometry recorded start to finish — boot → GARAGE tapped → every outfit tapped and PRO / ROOKIE on each (a per-rAF probe fails the run on any swap frame with the stage off, a procedural stand-in or no glTF rider = the ask 29/40 grey flash) → ‹ Menu → the same page in `?harness=1` → b1 from the Rookie golden as Street mustard, then from the Pro golden as Race blue/white, paced to real time, the finish hash checked against the node replay (and the Pro pin). Exit = failure / flash / page error / hash drift. One device-pixel still per settled swap (`garage-<outfit>[-<bike>].png`); `--garage-quality` pins the garage tier for inspection (the phone's own is the governor's) | `out/hero-art-clip/{clip.webm,clip.mp4,sheet.jpg,log.json,garage-*.png}` |
| `npx tsx harness/e2e/garage-swap-clip.mts [--out DIR] [--engine webkit\|chromium] [--geom 874x330] [--dpr 3] [--rounds 2] [--hold-ms 900]` | **garage swap latency** (ask 50): boot → GARAGE → every outfit × bike combination tapped, the set `--rounds` times (first taps, then repeats). Per tap: the model requests it caused (ms from the tap, bytes), `installMs` (tap → first frame with the new document installed, per rAF), the longest rAF gap until 500 ms after the install, gaps ≥ 34 ms, and `debugInfo().heroSwap` when the renderer exposes it. Writes a per-tap `table.md` for the evidence README | `out/garage-swap-clip/{clip.webm,clip.mp4,sheet.jpg,log.json,table.md}` |
| `npx tsx harness/e2e/worldmap-stills.mts --url=<server> --out=DIR [--engine=chromium\|webkit] [--geom=932x430,844x390,1280x720] [--seed=1] [--last=<trackId>] [--states=open]` · `worldmap-clip.mts --url --out [--engine] [--geom=932x430]` · `worldmap-frames.mts --url --out [--geom]` | **the world map (level select, ask 54)**: stills + a `measure()` per state (open → drag-pan two regions → pinch out to the continent → tap Night City → tap the locked H1 → the road back to B1): the camera (zoom, far, focused track / region, markers on screen, region plates decoded), scroll axes (must be none), page overflow, tappables under 44 px, overlaps, the rule on the plate / card / gate / pill; the played clip (menu → PLAY → the same path → RIDE → 3 s → pause → Quit) with the taps to B1 and the plate bytes; rAF frame intervals idle / drag / inertia / pinch + the cost of one camera push. Build `dist/` first (`npx vite build`) and serve it (`npx vite preview --port <n>`) | `<out>/{<engine>-<geom>-<n>-<state>.png,<engine>-report.json}`, `<out>/<engine>-<geom>-clip.{webm,json}`, `<out>/frames-<geom>.json` |
| `npx tsx harness/e2e/hero-boot-clip.mts [--out DIR] [--engine webkit\|chromium] [--geom 874x330] [--dpr 3] [--track b1-first-ride] [--ride-s 70]` | **played phone-tier boot clip** (ask 43 round 4): cold boot → menu held → the track launched → ridden on a held GAS pointer (real time, the game's own respawn) → results → MENU → GARAGE tapped. Logs every model / chunk request with the phase at its start and whether it was before `ready`, the renderer's hero-document slots per rAF (fills with their phase), every rAF gap ≥ 40 ms. Exit 1 when boot fetched a non-LOD hero, a slot filled while riding, a ≥ 100 ms gap landed mid-run, or garage entry fetched anything | `out/hero-boot-clip/{clip.webm,clip.mp4,sheet.jpg,log.json}` |
| `npx tsx harness/hero-capture.mts <build> <recording> <out> <fromTick> <toTick> [quality] [outfit] [fps] [WxH] [swiftshader\|metal\|webkit]` | **byte-proof hero capture**: one screenshot per rendered frame over an exact tick window, every served file hashed against the frozen build and every `.glb` body hashed in-page against `model-catalog.json` (the files are `heroFiles(outfit, header.bike)` from `src/render/hero/urls.ts` — the live hero family, per outfit and per class) | `<out>/{clip.mp4,frames/,evidence.json}` |
...
| `harness:e2e` | the boot suite alone first (throttled network on the wall clock; the **3G rows are informational** since the boot fetches all 14 hero files, 24.7 MB ≈ 9 min at 48 KB/s — the 3G / SW-off boot is sampled for 2 min, its rate and projected total / time logged, B1–B5 printed as notes; B3 stays a hard check on every LTE row), then bench, desktop, review, front/run/entry/hitrects per geometry and the transition grid split by transition (`flowGrid(..., { k, of })`) as parallel Playwright contexts | `(cores - 2) / 3` |
...
# Frozen build of a commit (the ship round, hero-ship.mts, any "on HEAD not the tree" number):
git archive <sha> | tar -x -C <scratch>; ln -s "$PWD/node_modules" <scratch>/node_modules
(cd <scratch> && VERCEL_GIT_COMMIT_SHA=<full sha> npx vite build --logLevel error)   # -> <scratch>/dist stamped <sha>
# vite.config refuses to stamp a build without a sha and an archive has no .git: VERCEL_GIT_COMMIT_SHA is the only
# way to name it there (it is not a vercel-only variable). hero-ship.mts / hero-capture.mts take <scratch>/dist directly;
# the gate serves dist/, so copy <scratch>/dist over it (after `trash dist`) and run the gate without --build.
pnpm harness:gate --only=camera                          # one section's row again (PARTIAL verdict, ship-gate.partial.json)
```
