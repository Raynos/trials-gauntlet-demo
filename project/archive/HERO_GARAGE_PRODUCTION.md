...
Closed: 2026-09-17 · ask 43 — the R33/R34 delivery is integrated (`3af533e` … `45f2e71`), the delivery, masters and handoff moved to `assets/blender/hero-art/` + `docs/evidence/hero-art/delivery/` (`4f27f47`), and `prototypes/hero-garage/` was retired. Remaining art quality (collar/panel tailoring, likeness, Race helmet remaster) is the user's call via `project/human-in-the-loop/QUEUE.md` HR-09.
...
## R34 — Existing design variants (in progress)

The user expanded the art scope to all five existing outfits and five bike skins. This supersedes the earlier no-extra-outfits limit for the existing designs only. Canonical rider IDs are street-mustard, street-charcoal, street-openface, race-bluewhite and race-charcoalyellow. Preserve genuine Street/open-face/Race geometry and palettes; do not count five hoodie recolours as five outfits. Game integration remains Claude + Opus owned.

Current source inspection finds only two bike liveries, Rookie and Pro (`src/render/bike/livery.ts`, `assets/blender/build_bike.py`). Build these two while asking the user to identify the other three; do not invent designs and claim they already existed. Usage started at 6% remaining, with the same hard 2% stop. Deliver variant exports and recorded Three.js evidence separately from the reviewed target01 catalog/master.
