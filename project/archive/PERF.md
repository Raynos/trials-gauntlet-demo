...
**Closed 2026-09-15** (close-out contract, `project/archive/CLOSEOUT.md`). The archivable claim: the **phone-high tier is live** (cut #3: LDR to the canvas at DPR 1.5, hero-only shadow, emissive-only bloom — model 8.8 ms on b1, WebKit/Metal ≤ medium), the **60-cap governor is live**, **bench + WebKit gate are in the harness** (`pnpm harness:bench`, `harness/bench/webkit.ts`, `pnpm harness:hero-webkit` — now ship-gate G11 rows), the **ledger runs through cut #4b** (rows 1–6 below: program churn, static-frame skip, static-world matrices, phone-high, same-material bake, the container skin array), and **every remaining cut is in `docs/plans/PERF-BACKLOG.md`** with its factor ÷ effort and the measurement that proves it. This file is a closed ledger from here; the loop continues from the backlog.
...
| 5 | (pending) | **#4 draw calls, part 1** — `world/props.ts buildBatches`: batches that share one material instance (and the same tier / shadow rule) bake into one mesh per chunk (matrices into positions, instance colours into vertex colours; per-chunk culling kept; `PropBatch.MERGE` is the A/B switch); unnamed world meshes (gate plaques) stop casting under the hero-only shadow | bench medians (phone): phone-high b1 **148 → 134**, h3 **171 → 147**, e2 114 → 104; medium 169 → 155 / 212 → 191; low 118 → 104 / 129 → 109; desktop high (diag) 202 → 186. Model phone-high b1 9.3 → 9.0 ms. In-page A/B merge on/off at t400/800/1200: desktop low 90 → 78, phone-high 145 → 130, desktop high 188 → 174 calls with ≤ 353 of 921 600 px differing, ≤ 1 px beyond 8/255 (an edge crack); the cut-3 → cut-4 still diffs (1 100 px on low) are Astra's arms fix 2b48370, in the rider's bbox. WebKit p50 low 1.66 / medium 2.62 / phone-high 2.66 at loadavg 26–30 (cut 3 ran at ~11: 1.60 / 1.92 / 1.88) — phone-high ÷ low = **1.18 in both runs**. Targets ≤ 100 / ≤ 120 **not reached**: what is left per frame on b1 is the 8 per-skin container batches (~20 draws: an atlas + per-instance UV offset, `#4b`), the 3-chunk neighbour visibility of 80 m chunks (~15), decals / volumetrics with distinct textures (~15), 16 hero + rider shadow draws (hero, Astra), gates (~14) | det pairs 15/15 on phone-high / medium / low and desktop high / low; churn 0; b1 clip camera PASS 0/2553 (riding 0, clamped 0) | |
| 6 | (pending) | **#4b container skin array** — `world/skinArray.ts` (+ `hall.ts`, `props.ts PropBatch.SKIN_ARRAY`, `util/materialKinds.ts`, `index.ts`): the hall's eight 1024×512 container skins become the eight layers of ONE `DataArrayTexture` on ONE derived `container` material (`sampler2DArray` + per-vertex `aSkin` through a 4-line `onBeforeCompile`; the depth twin strips its map sample so the shadow pass never binds the array through a `sampler2D`), so the eight per-skin batches fall into `buildBatches`' same-material bake → one draw per chunk. An array, not the designed flat 2048² atlas: `containerGeometry` repeats u across [0, 2], and a flat atlas needs `fract()` + tile padding and bleeds the neighbour tile at every wrap seam — the array keeps hardware repeat and per-layer mips, so the texels are the ones the separate skins gave. Same bytes (22 MB, `low` shrinks the layers to 512×256 like the canvases — `shrinkSkinArrays` at the world build and in `applyQuality`). `__setSkinArray(on)` is the A/B switch | bench medians, phone geometry, **same tree, switch off → on**: phone-high b1 **136 → 123** calls (tris 92 k → 92 k, model 9.1 → 8.8 ms), h3 **153 → 150** (the foundry has containers in its mid row only); medium b1 **166 → 143** (the 1024² map draws them twice), h3 185 → 180; low b1 **105 → 92**, h3 113 → 110. In-page A/B at t400/800/1200: phone-high b1 122 → 110 / 155 → 143 / 130 → 119, medium 163 → 144 / 204 → 184 / 174 → 150, low 92 → 80 / 117 → 105 / 96 → 85, desktop high 153 → 141 / 207 → 191 / 174 → 152; tris +0.1–1.6 k per frame (one merged chunk sphere is coarser than eight — backlog #1's chunk-range culling recovers it). Programs unchanged in kind (one scene + one depth program for the eight skins, as before). The bench's `tex MB` read 67 → 48 on this row's first run: the stat counted one layer of the array — fixed (`× depth`), the bytes are identical by construction | **A/B pixels** (skin array off vs on, same page): phone-high b1 ≤ 367 of 648 945 px differ, ≤ 7 px beyond 8/255, max 34; medium ≤ 291 / ≤ 2; low ≤ 139 of 288 420 / ≤ 4; h3 ≤ 23 px / 0 beyond 8; desktop high ≤ 1 969 of 921 600 (0.2 %) / ≤ 6 — the baked-position edge-crack class of row 5, no texel change (GL error 0 on every capture). **det pairs 15/15** on phone-high, medium and low at 874×330@3 (`out/bench/det/cut4b-phone`, t800 md5 6c7e63bc / 1dc35b06 / f5f110c3). **hero-webkit PASS, drift max 2.1 mm** (G11 row unchanged). WebKit ANGLE-Metal (`bench/webkit.ts`, b1 phone, loadavg 25–40): low 2.10 / medium 3.32 / phone-high 2.62 ms p50 — phone-high ÷ low 1.25 (rows 4–5: 1.18), phone-high still ≤ medium. The A/B is a harness command now: `pnpm exec tsx harness/bench/ab.ts --switch skinArray|merge` | the remaining terms on b1 phone-high: draws 123 × 0.02 = 2.5 ms, fill 1.14 × 4.4 = 5.0 ms; the next −20 draws are backlog #1 (chunk-range culling + gates) and #2 (decal array, −6) |
| 5b | (pending) | **cut #1 fix**: the skipped-frame path no longer resets `renderer.info` — `debugInfo().calls / tris` keep the last DRAWN frame (garage e2e `debugInfo calls: 0` 1/505) | idle garage low: calls 103 / 86 k tris reported with 4 drawn frames per 2.5 s (was 0 / 0); riding rows unaffected | typecheck + lint; idle bench | **#4b design (next):** one 2048×2048 atlas of the 8 container skins (4×2 of 1024×512) in `render/art`, `containerGeometry` UVs scaled ¼ × ½, a per-instance UV offset through `instanceColor`'s unused channel is not enough (3 floats, colour used) → a small `InstancedBufferAttribute` `aUvOffset` + a 3-line `onBeforeCompile` on the container material; then the 8 batches merge into one → b1 −20 draws (134 → ~114), h3 similar; the sign / poster decals (6 distinct textures) the same way afterwards (−6) |
