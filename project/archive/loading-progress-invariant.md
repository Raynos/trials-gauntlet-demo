...
## 7. Result (2026-09-15, core-game)

**Built as designed in §4, with two fractions instead of one** (the user's decision of 2026-09-15: "B Odometer" with
DOWNLOAD and SETUP kept as parallel tracks). Both are arithmetic identities over one typed table
(`src/boot/steps.ts`), both non-decreasing by construction, both exactly 1 on `done()` by `Σx/Σx`, and neither can
disagree with the other about *what* is needed because "needed" is not a value anywhere: a byte source is in
DOWNLOAD because it is in `BYTE_INFO`, and it is complete because the step that awaited it (`closedBy`) completed.
The full description is docs/design/game.md §12; the pieces, against the definition of done:

| §6 item | where | note |
|---|---|---|
| `createBootPlan`, authored step table, generated byte table | `src/boot/plan.ts`, `src/boot/steps.ts`, `src/boot/plan.generated.ts` (+ `totals.ts`) | generated file is committed and deterministic (sorted keys, no timestamp); regenerated by every `vite build` / `vite dev`; `pnpm typecheck` needs no build |
| `main.ts` is `plan.step(...)` all the way down; `done()` only on the exhausted type | `src/main.ts` `bootFront` | verified negatively: an early `done()` is "This expression is not callable. Type 'never'"; a repeated step is "not assignable" |
| `prepare()` takes the plan's runner; `this.report` and its rebind gone; art library and glTF report bytes | `src/render/index.ts`, `src/render/art/library.ts`, `src/render/hero/gltf.ts` | `prepare(run: StepRunner<PrepareStep>)`; readers get `ByteProgress` at construction (`heroBytes`, `artBytes`); `render()` no longer self-starts `prepare()` |
| inline script generated from TypeScript; Resource Timing machinery deleted; string api deleted | `src/boot/inline.ts` (+ `render.ts`, `stream.ts`, `handoff.ts`), `index.html`, `vite.config.ts` | 8.2 KB minified, asserted ≤ 8 KB by the build; `watchBackground`, `needBytes`, `bg`, `byTail`, `take`, buffer sizing, the `done()` reclassification, `getLoader`/`plan(n)`/`step(name)`/`progress(name)`/`fontsReady`/`loadManifestItem` are gone |
| property test + jsdom render test; headless boot e2e in four configurations | `src/boot/plan.test.ts`, `src/boot/render.test.ts`, `src/boot/totals.test.ts`, `harness/e2e/boot.mts` (`pnpm harness:e2e --only=boot`) | numbers below |
| no `setTimeout` race decides a step | — | art 1.5 s cap → needed step `bootArt`; key art 3 s + 2.5 s → background (`after`); fonts 4 s cap → `document.fonts.ready` uncapped; the 300 ms crossfade removal is the only timer left and nothing waits on it |
| phone clip | — | not in this round (headless only); the boot e2e is the gate's instrument until the clip is taken |

**Two fractions, reconciled with §4.4 "one number".** SETUP is §4.4's number (weights × step fractions).
DOWNLOAD is the same construction over byte sources: declared totals (from the build), deltas from the readers,
closure by the awaiting step. The one place the two could have disagreed — "DOWNLOAD says done, SETUP still waits
for bytes", or the reverse (f) — cannot arise because a source's closure *is* its step's completion. What a viewer
can still see is DOWNLOAD at 100 while SETUP compiles shaders, which is true.

**One fact the property test found while being written** (and the design absorbed): a running step that reported
`set(total, total)` counted as its full weight, so SETUP could read 100 while the last step was still resolving —
incident (d) in miniature. A step's work is now its `total` units *plus its completion* (`done / (total + 1)` while
running); only `work()` resolving credits the whole weight. Constant-free, and the test holds it.

**Property test** (`plan.test.ts`): 3 000 random sequences, 306 516 events (≤ 175 per sequence), 194 856 published
views checked for both monotonicity conditions; 300 paired runs for "after never moves a fraction"; plus the
throw cases (early `done()`, repeated step, delegate skipping a key, delegate running a foreign key) and the late-report
case (a `StepProgress` kept after its step completed changes nothing).

**Headless boot e2e** (`harness/e2e/boot.mts`, dist at 6bcffdf+, Chromium/SwiftShader, portrait 430×932):

| network | SW | art pack | monotone | final | DOWNLOAD 100 at (wall / live) | loader gone (main-thread freezes) | samples | result |
|---|---|---|---|---|---|---|---|---|
| LTE | off | present | y | 100/100 | 9.1 s / 8.7 s | 19.2 s (2 freezes, 5.0 s) | 112 | pass |
| LTE | on | present | y | 100/100 | 1.8 s / 0.4 s | 8.9 s (3 freezes, 5.9 s) | 54 | pass |
| 3G | off | present | y | 100/100 | 64.4 s / 58.1 s | 67.8 s (3 freezes, 8.0 s) | 1131 | pass |
| 3G | on | present | y | 100/100 | 2.0 s / 0.4 s | 9.7 s (3 freezes, 6.3 s) | 61 | pass |
| LTE | off | absent | y | 100/100 | 3.6 s / 2.3 s | 10.8 s (2 freezes, 6.9 s) | 68 | pass |
| LTE | on | absent | y | 100/100 | 2.3 s / 1.4 s | 8.1 s (3 freezes, 5.3 s) | 49 | pass |
| 3G | off | absent | y | 100/100 | 38.1 s / 32.5 s | 40.6 s (2 freezes, 5.4 s) | 646 | pass |
| 3G | on | absent | y | 100/100 | 3.6 s / 2.1 s | 11.0 s (2 freezes, 7.2 s) | 63 | pass |

Wall times are SwiftShader's (the first rAF takes ~2 s in this Chromium; the WebGL context and the first track freeze
the main thread for seconds — reported as `freezes`), not a phone's. The stuck detector judges only live page time,
and only the class of bug this document is about: DOWNLOAD unchanged for > 2 s while bytes it counts arrived at the
browser. Its first run caught a real flaw in the *first* version of this work — the boot art download started only
when `prepare()` ran, ~1.5 s late on a phone — which is why both needed downloads now start in the renderer constructor.

**Two costs the e2e measured that are not lies, for their owners:** `setModels` loads bike then rider sequentially
(`src/render/index.ts`), and `after` traffic (menu art, key art, per-track decals) shares the link with the needed bytes
— on the throttled LTE the needed 3.6 MB got roughly half the bandwidth. Both are wall time; neither touches the numbers.

**What the invariant closes, checked against §5 once more:** (a) no reporter to lose — `prepare()` takes the runner;
(b) rows are `BootStep` keys, integers only, weights in one table; (c) key art is `after` by construction; (d) `after`
never done-styled, 100 only at fraction 1; (e) no Resource Timing, no matching; (f) one DOWNLOAD number moves with
the glb bytes at read-chunk granularity and the done-time reclassification does not exist — the number *cannot* snap
to 100 at the crossfade because there is no code path that changes a denominator.
