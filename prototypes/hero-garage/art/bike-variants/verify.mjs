...
d.materials=[];d.images=[];d.textures=[];if(d.extensions)delete d.extensions.KHR_materials_variants;
...
const variant=process.argv[2]??'pro';assert(['rookie','pro'].includes(variant));const file=`public/assets/variants/bike-${variant}-art.glb`,original=fs.readFileSync(file);const gltf=await load(file);
