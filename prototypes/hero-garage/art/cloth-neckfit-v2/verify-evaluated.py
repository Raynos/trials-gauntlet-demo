...
P=Path.cwd()/'prototypes/hero-garage';src=P/'public/assets/street01-rider-neckfit-v2.glb';raw=src.read_bytes();n=struct.unpack_from('<I',raw,12)[0];j=json.loads(raw[20:20+n]);blob=raw[28+n:];keep={'rider:anatomical sweatshirt','rider:anatomical sweatshirt pocket','Street01_Authored_EditableBody_Runtime','fitted drawcord -1','fitted drawcord 1','collar eyelet -1','collar eyelet 1'};oldmeshes=j['meshes'];j['meshes']=[]
for node in j['nodes']:
...
arm=next(o for o in bpy.data.objects if o.type=='ARMATURE');shirt=bpy.data.objects['rider:anatomical sweatshirt'];hood=bpy.data.objects['Street01_Authored_EditableBody_Runtime'];actions=list(bpy.data.actions)
...
def sample():
 a,at=mesh(shirt);b,bt=mesh(hood);ta=BVHTree.FromPolygons(a,at,all_triangles=True);tb=BVHTree.FromPolygons(b,bt,all_triangles=True);pairs=ta.overlap(tb);cross=[];cross_heights=[]
 for i,k in pairs:
  ...
  for verts,ids,target,other in ((a,ia,b,ib),(b,ib,a,ia)):
   ...
   if hitpair:break
  if hitpair:
   cross.append([i,k]);head_inverse=(arm.pose.bones['head'].matrix@arm.data.bones['head'].matrix_local.inverted()).inverted();rest=head_inverse@hit;cross_heights.append((rest-Vector((.79,0,1.235))).dot(Vector((.40673664,0,.91354546))))
 signed=[];dist=[]
 for p in b:
 ...
 return {'crossingAxialHeightRangeM':[min(cross_heights),max(cross_heights)] if cross_heights else None,'crossingsAboveRestRimPlane':sum(h>.0264186 for h in cross_heights),'cordChecks':cordchecks,'shirtWorldBounds':[[min(v[k] for v in a) for k in range(3)],[max(v[k] for v in a) for k in range(3)]],'shirtTriangles':len(at),'headTriangles':len(bt),'broadphasePairs':len(pairs),'actualTriangleCrossings':len(cross),'firstCrossingPairs':cross[:10],'headMinimumDistanceM':min(dist),'headMinimumSignedDistanceM':min(signed),'headVerticesInsideLocalNormal':sum(x< -1e-6 for x in signed),'shirtDegenerateTriangles':sum(x<1e-12 for x in sa),'headDegenerateTriangles':sum(x<1e-12 for x in ha),'minimumShirtTriangleArea':min(sa),'minimumHoodTriangleArea':min(ha),'nonfiniteVertices':sum(not all(math.isfinite(c) for c in v) for v in a+b)}
rows=[]
