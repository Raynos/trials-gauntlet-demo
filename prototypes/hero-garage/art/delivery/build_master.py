...
seated=bpy.data.actions['sit_cruise'];rig.animation_data.action=seated
if seated.slots:rig.animation_data.action_slot=seated.slots[0]
...
report={'catalogSHA256':hashlib.sha256((P/'public/assets/catalog.json').read_bytes()).hexdigest(),'output':str(out.relative_to(R)),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'assets':records,'defaultAction':'sit_cruise','allActionsRetained':True,'sixClipDurationChecks':clip_checks,'temporaryDecodedFilesCleaned':True,'actions':[{'name':a.name,'frames':list(a.frame_range)} for a in bpy.data.actions],'coordinateSpace':'Blender Z up, meters, +X forward. Catalog glTF Y placement converted to Blender Z.','limitations':['Consolidated imported edit/review scene; canonical exports remain component build recipes.','Studio lighting is Blender-native and not a pixel-identical match to Three.js.','Dense groom retained without reduction.']}
