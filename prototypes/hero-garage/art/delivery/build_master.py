...
for asset in catalog['assets']:
    ...
    records.append({'kind':asset['kind'],'source':asset['url'],'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'objects':len(objects),'rigs':[o.name for o in objects if o.type=='ARMATURE']})
...
report={'output':str(out.relative_to(R)),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'assets':records,'actions':[{'name':a.name,'frames':list(a.frame_range)} for a in bpy.data.actions],'coordinateSpace':'Blender Z up, meters, +X forward. Catalog glTF Y placement converted to Blender Z.','limitations':['Consolidated imported edit/review scene; canonical exports remain component build recipes.','Studio lighting is Blender-native and not a pixel-identical match to Three.js.','Dense groom retained without reduction.']}
