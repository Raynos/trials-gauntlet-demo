...
R=Path.cwd();P=R/'prototypes/hero-garage';source=P/'art/delivery/hero-garage-master.blend';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();before=sha(source)
bpy.ops.wm.open_mainfile(filepath=str(source));scene=bpy.context.scene
catalog=json.loads((P/'public/assets/catalog.json').read_text());record=json.loads((P/'reports/delivery-master.json').read_text());assert record['sha256']==before;assert record['catalogSHA256']==sha(P/'public/assets/catalog.json')
assert scene.render.fps==30
for name in ['RIDER','BIKE','STUDIO - exclude from asset export']:assert name in bpy.data.collections
rigs=[o for o in bpy.data.collections['RIDER'].objects if o.type=='ARMATURE'];assert len(rigs)==1;rig=rigs[0];default=rig.animation_data.action.name;assert default=='sit_cruise';assert all(t.mute for t in rig.animation_data.nla_tracks)
...
(P/'reports/delivery-master-reopen.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
