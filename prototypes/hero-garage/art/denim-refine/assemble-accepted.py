...
base=Path(args.base) if args.base else P/'public/assets/street01-rider-tailored-footwear.glb';new=P/'public/assets/street01-denim-refined-accepted.glb';j,basebin=read(base);h,hbin=read(new);original=copy.deepcopy(j);blob=bytearray(basebin)
...
out=Path(args.out) if args.out else P/'public/assets/street01-rider-art27-denim-accepted.glb';out.write_bytes(data)
assert bytes(blob[:len(basebin)])==basebin;assert j['animations']==original['animations'];assert j['skins']==original['skins']
r={'independentWorldRestMatrixMaxDelta':rest_delta,'inverseBindMatchTolerance':.00001,'removedSourceDenimTriangles':matched,'sourceDenimTriangles':len(source),'oldBodyBinaryPrefixIdentical':True,'savedHeadHoodieAndFootwearMeshesAndAttributesIdentical':j['meshes'][1:meo]==original['meshes'][1:],'sixAnimationsAndSkinJSONIdentical':True,'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'reports/denim-refine-accepted-assembly.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
