...
import bpy,json,struct,copy,hashlib,itertools,sys,argparse
import numpy as np
...
base=Path(args.base).resolve();new=P/'public/assets/street01-denim-textured.glb';j,basebin=read(base);h,hbin=read(new);original=copy.deepcopy(j);blob=bytearray(basebin)
...
r={'source':str(base),'output':str(out),'independentWorldRestMatrixMaxDelta':rest_delta,'inverseBindMatchTolerance':.00001,'removedSourceDenimTriangles':matched,'sourceDenimTriangles':len(source),'oldBodyBinaryPrefixIdentical':True,'savedHeadHoodieAndFootwearMeshesAndAttributesIdentical':j['meshes'][1:meo]==original['meshes'][1:],'sixAnimationsAndSkinJSONIdentical':True,'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'reports/denim-texture-assembly.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
