...
target_nodes={n['name']:i for i,n in enumerate(j['nodes']) if 'mesh' in n and n.get('name') in ('rider:cotton_laces',)}
assert len(target_nodes)==1,target_nodes
...
check_nodes=copy.deepcopy(j['nodes'])
for ni in target_nodes.values():check_nodes[ni]['mesh']=original['nodes'][ni]['mesh']
assert check_nodes==original['nodes']
r={'allNodesExactExceptLaceMeshReference':True,'independentWorldRestMatrixMaxDelta':rest_delta,'oldBinaryPrefixIdentical':True,'oldMeshesUnchanged':j['meshes'][:meo]==original['meshes'],'sixAnimationsAndSkinJSONIdentical':True,'replacedNodes':list(target_nodes),'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'reports/footwear-lacing-assembly.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
