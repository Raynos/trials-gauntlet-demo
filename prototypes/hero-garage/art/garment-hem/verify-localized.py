...
P=Path.cwd()/'prototypes/hero-garage';src=P/'public/assets'/os.environ.get('SLEEVE_STUDY_SOURCE','street01-rider-hem-study.glb');raw=src.read_bytes();n=struct.unpack_from('<I',raw,12)[0];j=json.loads(raw[20:20+n]);blob=raw[28+n:];keep={'rider:anatomical sweatshirt','rider:anatomical sweatshirt pocket','rider:folded hood','fitted drawcord -1','fitted drawcord 1','collar eyelet -1','collar eyelet 1'};oldmeshes=j['meshes'];j['meshes']=[]
for node in j['nodes']:
 if 'mesh' not in node:continue
 if node.get('name')=='rider' and any('indigo denim' in j['materials'][pr['material']].get('name','') for pr in oldmeshes[node['mesh']]['primitives']):node['name']='diagnostic-denim';keep.add('diagnostic-denim')
...
origin=arm.data.bones['pelvis'].head_local.copy();up=(arm.data.bones['spine'].tail_local-arm.data.bones['spine'].head_local).normalized();front=Vector((up.z,0,-up.x));hem_ids={v.index for v in shirt.data.vertices if (v.co-origin).dot(up)<.18 and (v.co-origin).dot(front)<.055 and abs(v.co.y)<.23}
...
def sample():
 a,at=mesh(shirt);b,bt=mesh(hood);dv,dt=mesh(bpy.data.objects['diagnostic-denim']);denimtree=BVHTree.FromPolygons(dv,dt,all_triangles=True);denimcross=intersection_count(a,at,dv,dt);hemtri=[t for t in at if all(i in hem_ids for i in t)];hemcross=intersection_count(a,hemtri,dv,dt);hempoints=[a[i]for i in hem_ids];hemdist=[denimtree.find_nearest(v)for v in hempoints];hemsigned=[(p-hit).dot(n)for p,(hit,n,_,_)in zip(hempoints,hemdist)];denimgap=min(denimtree.find_nearest(v)[3]for v in a);ta=BVHTree.FromPolygons(a,at,all_triangles=True);tb=BVHTree.FromPolygons(b,bt,all_triangles=True);pairs=ta.overlap(tb);cross=[]
 for i,k in pairs:
 ...
 return {'hemTriangles':len(hemtri),'hemVertices':len(hem_ids),'hemDenimCrossings':hemcross,'hemMinimumGapM':min(d[3]for d in hemdist),'hemMinimumSignedGapM':min(hemsigned),'hemInsideVertices':sum(v<0 for v in hemsigned),'denimTriangleCrossings':denimcross,'minimumShirtDenimVertexGapM':denimgap,'elbowSection':elbows,'cordChecks':cordchecks,'shirtWorldBounds':[[min(v[k] for v in a) for k in range(3)],[max(v[k] for v in a) for k in range(3)]],'shirtTriangles':len(at),'hoodTriangles':len(bt),'broadphasePairs':len(pairs),'actualTriangleCrossings':len(cross),'firstCrossingPairs':cross[:10],'hoodMinimumDistanceM':min(dist),'hoodMinimumSignedDistanceM':min(signed),'hoodVerticesInsideLocalNormal':sum(x< -1e-6 for x in signed),'shirtDegenerateTriangles':sum(x<1e-12 for x in sa),'hoodDegenerateTriangles':sum(x<1e-12 for x in ha),'minimumShirtTriangleArea':min(sa),'minimumHoodTriangleArea':min(ha),'nonfiniteVertices':sum(not all(math.isfinite(c) for c in v) for v in a+b)}
rows=[]
