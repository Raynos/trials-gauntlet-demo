...
P=Path.cwd()/'prototypes/hero-garage';src=P/'public/assets'/os.environ.get('SLEEVE_STUDY_SOURCE','street01-rider-garment-shape-study.glb');raw=src.read_bytes();n=struct.unpack_from('<I',raw,12)[0];j=json.loads(raw[20:20+n]);blob=raw[28+n:];keep={'rider:anatomical sweatshirt','rider:anatomical sweatshirt pocket','rider:folded hood','fitted drawcord -1','fitted drawcord 1','collar eyelet -1','collar eyelet 1'};oldmeshes=j['meshes'];j['meshes']=[]
for node in j['nodes']:
