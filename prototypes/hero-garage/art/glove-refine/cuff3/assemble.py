"""Replace only source glove triangles; preserve accepted identity and motion bytes."""
...
manifest=json.loads((P/'art/glove-refine/cuff3/source-removal-triangles.json').read_text())
...
r={'independentWorldRestMatrixMaxDelta':rest_delta,'inverseBindMatchTolerance':.00001,'removedSourceGloveTriangles':matched,'sourceGloveTriangles':len(source),'oldBodyBinaryPrefixIdentical':True,'savedHeadAndHoodieMeshesAndAttributesIdentical':j['meshes'][1:meo]==original['meshes'][1:],'sixAnimationsAndSkinJSONIdentical':True,'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'reports/glove-refine-cuff3-assembly.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
