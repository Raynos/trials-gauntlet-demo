...
target_nodes={n['name']:i for i,n in enumerate(j['nodes']) if 'mesh' in n and n.get('name') in ('rider:glove_top','rider:gloves','rider:hero glove construction')}
assert len(target_nodes)==3,target_nodes
...
(P/'art/glove-surface/assembly-report.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
