...
base=P/'public/assets/street01-rider-identity.glb';new=P/'public/assets/street01-cloth28-donor.glb';assert new.name=='street01-cloth28-donor.glb';j,basebin=read(base);h,hbin=read(new);original=copy.deepcopy(j);blob=bytearray(basebin)
...
out=P/'public/assets/street01-rider-cloth28.glb';assert out.name=='street01-rider-cloth28.glb' and out not in (base,new);out.write_bytes(data)
assert bytes(blob[:len(basebin)])==basebin;assert j['animations']==original['animations'];assert j['skins']==original['skins']
r={'removedSourceClothAndHighDrawcordTriangles':matched,'removedInterimCollarTriangles':collar,'sourceClothTriangles':len(source),'oldBodyBinaryPrefixIdentical':True,'savedHeadMeshesAndAttributesIdentical':j['meshes'][1:meo]==original['meshes'][1:],'sixAnimationsAndSkinJSONIdentical':True,'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'art/hoodie-finish/assembly-report.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
