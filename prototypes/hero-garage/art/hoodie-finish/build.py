...
R=Path.cwd();P=R/'prototypes/hero-garage';D=P/'art/hoodie-finish';sys.path[:0]=[str(R/'assets/blender'),str(P/'art/hoodie-shell')]
import common as C
from unwrap_cloth import unwrap_cloth
source=P/'art/hoodie-shell/hoodie-source.blend';digest=hashlib.sha256(source.read_bytes()).hexdigest();bpy.ops.wm.open_mainfile(filepath=str(source));arm=bpy.data.objects['rider_rig'];shirt=bpy.data.objects['rider:anatomical sweatshirt'];cloth=[o for o in bpy.context.scene.objects if o.type=='MESH'];uvreport=unwrap_cloth(cloth)
for o in cloth:
...
bpy.ops.wm.save_as_mainfile(filepath=str(D/'cloth28-source.blend'))
mat=C.atlas_material('mustard continuous cotton',paths)
for o in cloth:
 o.data.materials.clear();o.data.materials.append(mat)
 for p in o.data.polygons:p.material_index=0
for a in list(bpy.data.actions):bpy.data.actions.remove(a)
C.export_glb(str(P/'public/assets/street01-cloth28-donor.glb'),list(bpy.context.scene.objects),animations=True,meshopt=False)
assert hashlib.sha256(source.read_bytes()).hexdigest()==digest
r={'sourceSHA256':digest,'sourceUnchanged':True,'uv':uvreport,'inwardThicknessM':.002,'thicknessAfterBake':True,'mustardLinearRGB':[.32,.155,.032],'cottonRoughness':.88,'drawcords':cord_report,'textures':paths,'limitations':['Candidate only; parent must inspect material appearance and drawcord movement.','Inner surface deliberately shares outer UVs; thickness is evaluated after bake to avoid overlapping bake writes.']};(D/'build-report.json').write_text(json.dumps(r,indent=2)+'\n');print('CLOTH28 DONE')
