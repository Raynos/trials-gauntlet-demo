...
for t in m.loop_triangles:
 if m.materials[t.material_index].name in ('hero heavy cotton','hero knitted rib'):source.append(np.array([tuple(uv[i].uv) for i in t.loops]))
...
base=P/'public/assets/street01-rider-identity.glb';new=P/'public/assets/street01-hoodie-shell.glb';j,basebin=read(base);h,hbin=read(new);original=copy.deepcopy(j);blob=bytearray(basebin)
...
out=P/'public/assets/street01-rider-tailored.glb';out.write_bytes(data)
assert bytes(blob[:len(basebin)])==basebin;assert j['animations']==original['animations'];assert j['skins']==original['skins']
r={'removedSourceClothTriangles':matched,'removedInterimCollarTriangles':collar,'sourceClothTriangles':len(source),'oldBodyBinaryPrefixIdentical':True,'savedHeadMeshesAndAttributesIdentical':j['meshes'][1:meo]==original['meshes'][1:],'sixAnimationsAndSkinJSONIdentical':True,'outputSHA256':hashlib.sha256(data).hexdigest(),'outputBytes':len(data)}
(P/'reports/hoodie-shell-assembly.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
