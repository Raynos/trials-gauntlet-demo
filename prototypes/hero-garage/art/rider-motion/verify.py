...
R=Path.cwd();P=R/'prototypes/hero-garage';p=P/'public/assets/street01-rider-motion.glb';r=json.loads((P/'reports/rider-motion.json').read_text())
...
for act in actions:
 ...
 for frame in frames:
  ...
  gaps=[]
  if act.name in ('compression','extension','landing_absorption'):
...
a,ab=glb(P/'public/assets/street01-rider-cloth.glb');b,bb=glb(p)
...
assert r['sourceTextureBytesIdentical'];assert r['triangleCounts']['source']==r['triangleCounts']['candidate']
