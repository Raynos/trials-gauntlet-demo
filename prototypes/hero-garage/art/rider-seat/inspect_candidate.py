...
R=Path.cwd();P=R/'prototypes/hero-garage';bpy.ops.wm.read_factory_settings(use_empty=True);bpy.context.scene.render.fps=30;bpy.ops.import_scene.gltf(filepath=str(P/'public/assets/street01-rider-seat.glb'));arm=next(o for o in bpy.data.objects if o.type=='ARMATURE');mesh=bpy.data.objects['rider']
...
print(json.dumps(rows,indent=2));(P/'art/rider-seat/candidate-grid.json').write_text(json.dumps(rows,indent=2)+'\n')
