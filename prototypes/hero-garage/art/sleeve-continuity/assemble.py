...
parser=argparse.ArgumentParser();parser.add_argument('--base');parser.add_argument('--out');args=parser.parse_args(sys.argv[sys.argv.index('--')+1:]if '--'in sys.argv else []);base=Path(args.base)if args.base else P/'public/assets/street01-rider-art28-final.glb';new=P/'public/assets/street01-sleeve-donor.glb';j,basebin=read(base);h,hbin=read(new);original=copy.deepcopy(j);blob=bytearray(basebin)
...
out=Path(args.out)if args.out else P/'public/assets/street01-rider-sleeve-study.glb';out.write_bytes(data)
...
(P/'art/sleeve-continuity/assembly-report.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
