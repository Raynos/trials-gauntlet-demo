...
Authoritative export URLs and SHA256: [manifest.json](manifest.json). Selection/review/master mapping: [selection.json](selection.json). Paths below are relative to `prototypes/hero-garage/public/assets/`.
...
[Variant guide](README.md), manifest-listed recorded WebKit clips and rig reports, plus [R35 review](../../reports/art-round35-review.json). All selected new rider assets were inspected through six clips under neutral/garage lighting. Rig audits sample750 frames; reopened Blender masters retain six actions and packed images. These are scoped checks, not proof of global intersection freedom or actual-device performance.
