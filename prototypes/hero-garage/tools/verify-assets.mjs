...
function dimensions(b) {
  if(b.subarray(0,12).equals(Buffer.from([0xab,0x4b,0x54,0x58,0x20,0x32,0x30,0xbb,0x0d,0x0a,0x1a,0x0a]))){
    const dfd=b.readUInt32LE(48),model=b[dfd+12];
    if(model!==166)throw new Error('Ledger supports UASTC KTX2 only');
    const width=b.readUInt32LE(20),height=b.readUInt32LE(24),levels=b.readUInt32LE(40);
    if(levels!==Math.floor(Math.log2(Math.max(width,height)))+1)throw new Error('KTX2 mip chain incomplete');
    return {width,height,levels,sourceFormat:'KTX2/UASTC',gpuEstimateFormat:'4x4 blocks, 16 bytes (device transcode dependent)',blockBytes:16};
  }
...
function inspect(file) {
  ...
  const textureRows=(doc.images??[]).map((im,index)=>{
    ...
    if(d.width&&d.height) { let w=d.width,h=d.height; resident=0; while(true){resident+=d.blockBytes?Math.ceil(w/4)*Math.ceil(h/4)*d.blockBytes:w*h*4;if(w===1&&h===1)break;w=Math.max(1,w>>1);h=Math.max(1,h>>1);} }
