...
export interface DeclaredBootTotals {
  heroModels: Record<RiderOutfit, number>;
  bootArt: number;
}
...
export function declaredBootTotals(bytes: (key: BootAssetKey) => number): DeclaredBootTotals {
  ...
  return {
    heroModels: {
      street: HERO_FILES_BY_OUTFIT.street.reduce((sum, file) => sum + need(file), 0),
      race: HERO_FILES_BY_OUTFIT.race.reduce((sum, file) => sum + need(file), 0),
    },
