 ...
 * backgrounds, thumbs, the other biomes' world art) and the world map's plates, **both** tiers — a phone
 * that rotates or is mirrored to a desktop must not need the radio to draw its continent.
 *
...
/** The world map's plates and index: not in the art manifest, `<img>`-loaded by `worldMapScreen.ts`. */
export function worldMapUrls(): string[] {
  const out = [worldMapIndexSrc()];
  for (const hi of [true, false]) {
    out.push(worldPlateSrc(hi));
