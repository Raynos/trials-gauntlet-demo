...
import { RIDER_PRESETS, riderPreset } from '../core/riderPresets';

vi.mock('./totals', () => ({ DECLARED_BOOT_TOTALS: { heroModels: { street: 220, race: 245, openface: 260 }, bootArt: 40 } }));
const totals = { heroModels: { street: 220, race: 245, openface: 260 }, bootArt: 40 };
...
describe('outfit-dependent boot totals', () => {
  it('matches runtime for every catalog design, legacy alias and invalid family/palette combination', () => {
    for (const stored of ['street-openface', 'street-charcoal', 'race-charcoalyellow', 'street', 'race', 'invalid']) {
      ...
      for (const requested of [...RIDER_PRESETS.map(p => p.id), 'street', 'race', 'street-bluewhite', 'race-mustard', 'invalid', '']) {
        ...
        const expected = riderPreset(loadRiderOutfit(requested)).family;
  ...
  it('uses exactly the main entry preference rules for inline and fallback plans', async () => {
    for (const [stored, search, expected] of [
      ...
      expect(inlineTotals.heroModels).toBe(totals.heroModels[riderPreset(expected).family]);
