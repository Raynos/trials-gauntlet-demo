...
export interface RiderBody {
  ...
  angVel: number;
  /**
   * Derived compatibility view of the physical body through the shared rider geometry.
   * Hips and angles come from the same COM inverse used by contacts, sensors and the hero;
   * this is not a separate authored stance or a clamped visual excursion. Coordinates
   * use the authored axle frame, x forward and y up (metres/radians). The source physical
   * body is hashed; this redundant derived view is not.
   */
  drawn?: RiderDrawnPose;
}

/** Derived physical pose for compatibility consumers (see `RiderBody.drawn`). */
