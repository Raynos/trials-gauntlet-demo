...
describe('GltfRider additive clips', () => {
  it('idle_breathe runs at rest and moves the shoulder within the clip amplitude', () => {
    const { rider, shoulder, shoulderRest } = build();
    let peak = 0;
    for (let i = 0; i < 120; i++) {
      rider.update(frameAtRest(i / 60));
      peak = Math.max(peak, shoulder.position.distanceTo(shoulderRest));
    }
    expect(peak).toBeGreaterThan(0.005); // the clip is alive (1 cm peak at t = 1 s)
    expect(peak).toBeLessThan(0.0101);
  });
