...
export const PROPS: { readonly [P in PropId]: { readonly zone: 'coast' | 'alpine' | 'quarry' | 'snowline'; readonly base: readonly ObstacleKind[]; readonly mesh: string } } = {
  ...
  log: { zone: 'alpine', base: ['drum', 'logpile', 'seesaw'], mesh: 'barked pine log lying across the course, cut end rings visible (not a cable spool); on a see-saw a log teetering on the jam' },
  'log-stack': { zone: 'alpine', base: ['box', 'ledge', 'ramp'], mesh: 'stacked log ends (a timber stack face) with chains' },
  ...
  'truck-bed': { zone: 'alpine', base: ['box', 'ramp'], mesh: 'logging truck trailer bed with bolsters and a log load behind the rider' },
  ...
  'ore-cart': { zone: 'quarry', base: ['box', 'seesaw', 'ramp'], mesh: 'rusted ore cart on a rail stub, rubble load; on a see-saw it rides a tipping rail section' },
  ...
  'rope-bridge': { zone: 'quarry', base: ['plank', 'box', 'ledge', 'ramp'], mesh: 'plank-and-rope bridge deck with hand ropes; slots between deck boards are the kill gaps' },
  ...
  'lift-tower': { zone: 'snowline', base: ['pole', 'box', 'ramp'], mesh: 'ski-lift tower: lattice pylon, the cap is its sheave-wheel platform' },
  ...
  fence: { zone: 'snowline', base: ['plank', 'ledge', 'box', 'ramp'], mesh: 'avalanche snow fence: timber slats on posts' },
};
