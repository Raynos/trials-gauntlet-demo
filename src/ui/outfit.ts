...
export const RIDER_OUTFIT_KEY = 'rockhop.riderOutfit';
...
function loadPreference<T>(override: string | null | undefined, normalize: (value: string | null | undefined) => T | null): T | null {
  ...
  try {
    const stored = persistentStorage()?.getItem(RIDER_OUTFIT_KEY);
...
export function saveRiderOutfit(outfit: RiderOutfit): void {
  ...
  if (!canonical) return;
  try {
    persistentStorage()?.setItem(RIDER_OUTFIT_KEY, canonical);
  } catch {
